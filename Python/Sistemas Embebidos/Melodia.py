...
# Pin para el buzze
buzzer = PWM(Pin(18))
# Los pines GPIO recomendados para PWM en ESP32 son:
# 2, 4, 5, 12–19, 21–23, 25–27, 32–33
# (Evita 34–39 porque solo son entrada)

# Melodía de ejemplo (tono duracion)
Cancion1 = [
...
Cancion2 = [
    ("C4", 0.3), ("E4", 0.3), ("F4", 0.3), ("G4", 0.3),
    ("C5", 0.5), ("B4", 0.25), ("A4", 0.25), ("G4", 0.5),
    ("F4", 0.25), ("E4", 0.25), ("D4", 0.5),
    ("E4", 0.25), ("F4", 0.25), ("G4", 0.5), ("C4", 0.5),
    ("P", 0.2),  # Pausa
    ("C4", 0.3), ("E4", 0.3), ("F4", 0.3), ("G4", 0.3),
    ("C5", 0.5), ("B4", 0.25), ("A4", 0.25), ("G4", 0.5),
    ("F4", 0.25), ("E4", 0.25), ("D4", 0.5),
...
def play_note(nota, duracion):
    if nota == "P":  # Pausa 
        buzzer.duty_u16(0)
        sleep(duracion)
    else:
        freq = tones.get(nota, 0)
        if freq > 0:
            buzzer.freq(freq)
            buzzer.duty_u16(32768)  # 50% ciclo útil
            sleep(duracion)
            buzzer.duty_u16(0)
            sleep(0.05)

# Ejecutar melodía
for nota, duracion in Cancion1:
    play_note(nota, duracion)

for nota, duracion in Cancion2:
    play_note(nota, duracion)
# Apagar buzzer
buzzer.deinit()
